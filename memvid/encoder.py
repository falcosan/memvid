...
class MemvidEncoder:
    ...
    def _generate_qr_frames(self, temp_dir: Path, show_progress: bool = True, start_frame: int = 0) -> Path:
    ...
    def append_to_video(self, existing_video: str, existing_index: str, output_file: str, output_index: str,
        ...
        start_frame = max(existing_index_manager.frame_to_chunks.keys()) + 1 if existing_index_manager.frame_to_chunks else 0
        logger.info(f"Appending {len(self.chunks)} chunks starting from frame {start_frame} (chunk_id {last_chunk_id + 1})")
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if show_progress:
                ...
                logger.info(f"Total video duration: {stats.get('duration_seconds', 0):.1f} seconds")
